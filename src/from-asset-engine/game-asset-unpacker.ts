...
export const unpackGameAssets = (arrayBuffer: ArrayBuffer) => {
...
const bytesToPalettes = (arrayBuffer: ArrayBuffer, startingOffset = 0): UnpackedAsset => {
...
function bytesToBackgrounds(arrayBuffer: ArrayBuffer, startingOffset: number): UnpackedAsset {
    ...
    doTimes(numberOfBackgroundLayers, () => {
      ...
      doTimes(numberOfSpritesInBackground, () => {
        ...
        const [spriteIndex, pos] = splitByte(spriteByte, 3);
        backgroundLayer.sprites.push({ pos, spriteIndex });
      });
