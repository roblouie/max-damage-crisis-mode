...
export class DrawingEngineOld {
  ...
  clearContext(): void {
    this.canvasContext.cClearRect(0, 0, this.width, this.height);
  }
  ...
  drawSprite(spriteNumber: number, positionX: number, positionY: number) {
    ...
    this.canvasContext.cSave();
    this.canvasContext.scale(4, 4);
    this.canvasContext.cDrawImage(canvas, spriteNumber * 32, 0, sprite.width * 16, sprite.height * 16, positionX, positionY, sprite.width * 16, sprite.height * 16);
    this.canvasContext.cRestore();
  }
  ...
  private drawSpriteToCanvas(spriteNumber: number, positionX: number, positionY: number, ocContextNumber?: number) {
    ...
    sprite.spriteTiles.aForEach((spriteTile: SpriteTile, index: number) => {
      ...
      if (index === 0) {
        ...
        context.cPutImageData(imageData, positionX, positionY + this.tileSize);
      } else if (index === 1 && sprite.width === 2) {
        context.cPutImageData(imageData, positionX + this.tileSize, positionY);
      } else if (index === 2) {
        context.cPutImageData(imageData, positionX, positionY + this.tileSize);
      } else if (index === 3) {
        context.cPutImageData(imageData, positionX + this.tileSize, positionY + this.tileSize);
      }
    });
  }
  ...
  drawBackgroundLayerToBackgroundCanvases(backgroundIndex: number) {
    doTimes(3, (i: number) => {
      this.offscreenContexts[i].cClearRect(0, 0, 128, 256);
